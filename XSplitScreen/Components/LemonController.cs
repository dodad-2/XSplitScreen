...
namespace DoDad.XSplitScreen.Components
{
    class LemonController : UnityEngine.MonoBehaviour
    {
        ...
        internal static HGButton button;
        ...
        void Update()
        {
            if (isLemonized)
            {
                ...
                if (!button.hovering && !stayLemonFriend)
                    EndLemonization();
            }
            else
            {
        ...
        {
            stayLemonFriend = !stayLemonFriend;

            SetSecretState(stayLemonFriend);

            if (openURL)
                Application.OpenURL("https://www.patreon.com/posts/splitscreen-for-75856056?utm_medium=clipboard_copy&utm_source=copyLink&utm_campaign=postshare_creator&utm_content=join_link");
