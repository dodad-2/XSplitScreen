...
namespace DoDad.XSplitScreen.Components
{
    class AssignmentScreen : BaseMainMenuScreen
    {
        ...
        internal HGButton patreonHGButton { get; private set; }
        #region Unity Methods
        void Update()
        {
            return;
            if (patreonHGButton.hovering && !LemonController.isLemonized)
                lemonizer.BeginLemonization();

            if (!patreonHGButton.hovering && LemonController.isLemonized)
                lemonizer.EndLemonization();

            if (patreonHGButton.hovering && LemonController.isLemonized && patreonHGButton.hoverToken != LemonController.token)
                patreonHGButton.hoverToken = LemonController.token;

            if (!patreonHGButton.hovering && !LemonController.isLemonized && patreonHGButton.hoverToken != "XSS_OPTION_PATREON_HOVER")
                patreonHGButton.hoverToken = "XSS_OPTION_PATREON_HOVER";
        }
        ...
        private void CreateUI()
        {
            ...
            var optionPatreon = GameObject.Instantiate(XLibrary.Resources.GetPrefabUI("ScreenOption"));
            optionPatreon.name = "(Option) Patreon";
            optionPatreon.transform.SetParent(juicePanel);
            optionPatreon.transform.localScale = Vector3.one;
            optionPatreon.GetComponent<LanguageTextMeshController>().token = "XSS_OPTION_PATREON";

            patreonHGButton = optionPatreon.GetComponent<HGButton>();
            patreonHGButton.hoverToken = "XSS_OPTION_PATREON_HOVER";
            patreonHGButton.requiredTopLayer = layerKey;
            patreonHGButton.onClick.AddListener(OnClickPatreon);
            patreonHGButton.hoverLanguageTextMeshController = descriptionController;
            patreonHGButton.updateTextOnHover = true;

            LemonController.button = patreonHGButton;

            lemonizer.Lemonize(patreonHGButton.image);

            optionPatreon.SetActive(true);
        ...
        {
            lemonizer.OnClickPatreon();
            patreonHGButton.hoverLanguageTextMeshController.token = patreonHGButton.hoverToken;
        }
        #endregion
